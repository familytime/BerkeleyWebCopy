...
##
@primitive("connect")
def scheme_connect_list(s):
    t = []
    while not scheme_nullp(s):
        t.append(scheme_car(s))
        s = scheme_cdr(s)
    lengths = [i for i in t if isinstance(i, int)]
    colors = [i for i in t if isinstance(i, str)]
    result = scheme_cons(scheme_list(*lengths), scheme_list(scheme_list(*colors), nil))
    return result
...
def scheme_pix(s, i):
    ...
    while not scheme_nullp(s):
        t.append(scheme_car(s))
        s = scheme_cdr(s)
    return scheme_car(scheme_list(t[i]))
