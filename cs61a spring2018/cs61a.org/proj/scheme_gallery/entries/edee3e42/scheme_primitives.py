...
def tscheme_setup(w,h):
    return turtle.setup(width = w, height =h )
@primitive("delay")
def tscheme_delay(time):
    return turtle.delay(time)

@primitive("rgb_background")
def tscheme_rgb_background(r, g, b):
    """Screen height in pixels of the current size (default 1)."""
    _tscheme_prep()
    turtle.bgcolor(r, g, b)

@primitive("pen_width")
def tscheme_pen_width(size):
    """Screen height in pixels of the current size (default 1)."""
    turtle.pensize(size)
@primitive("rgb_color")
def tscheme_rgb_color(r, g, b):
    """Screen height in pixels of the current size (default 1)."""
    _tscheme_prep()
